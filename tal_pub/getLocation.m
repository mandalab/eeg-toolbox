...
% You can also skip cross validation.
%
% xyz   - looked up based on leads.csv file
% index - looked up based on SUMA surface
%
% REVISION HISTORY:
%   09/16 MST - Created
%   10/16 MST - Add fast param
%   03/17 MST - Added CT-registered param
%   10/17 MST - tal folder update--no longer look at monopolar.mat
%   07/18 MST - update to new directory structure
...
fnameLeadsBackup = fullfile(tal, 'leads.csv');
fnameLeads      = fullfile(tal, 'zloc/mr_pre/leads.csv');

assert(~isempty(find(strcmp(chanName, leads), 1)), 'chanName not found in leads: %s', chanName);
...
if ~fast && exist(fnameMonopolar, 'file') && USE_MONOPOLAR
...
if ~isempty(xyz) && ~isempty(xyz_monopolar)
    ...
    msg = ['Cross validation of leads xyz and monopolar failed:\n'...
        xyzFmt, ' ~= \n'...
        xyzFmt];
    assert(isequal(xyz, xyz_monopolar{1,:}), msg, xyz, xyz_monopolar{1,:});
end
...
sumaDir = fullfile(rootEEGdir, subj, 'tal/zloc/freesurfer', subj, 'SUMA');
...
if ~exist('surf', 'var') || isempty(surf)
    surf = surf_load_main('hemi', hemi, 'dir', sumaDir);
end

if strcmpi(type, 'utah') || isempty(ndx_monopolar)
    [index, dist] = knnsearch(surf.vertices, xyz);
    if isempty(ndx_monopolar)
        fprintf('Vertex found %d mm away\n', dist);
    else
        % cross validate
