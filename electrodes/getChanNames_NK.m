function [actualName_ALL actualCode_ALL]=getChanNames_NK(elecFileNK,eegFileNK)
%
% getChanNames.m
% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
f_elecFileNk = fopen(elecFileNK, 'r');
nkFileCells = textscan(f_elecFileNk,'%s%s','delimiter','='); % *MST textread -> textscan
[allCodes,allNames] = deal(nkFileCells{:});
fclose(f_elecFileNk);
...
for k=1:numChannels
    ...
    end %if it is a good electrode, append it to the jacksheet
...
nonEmptyNames = ~cellfun('isempty', actualName_ALL); % MST 07/17 add to not return '' entries
actualName_ALL = actualName_ALL(nonEmptyNames); % MST 07/17 add to not return '' entries
actualCode_ALL = actualCode_ALL(nonEmptyNames); % MST 07/17 add to not return '' entries
