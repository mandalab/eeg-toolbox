...
%
%
% 3/12/2020: SJ added 3 manual fixes from previous version (check if file exists, spacing fixes, changed "HACKY FIX" at end back to original with keyboard)
%
...
%- blackrock had an "intense" warning... JW is skipping that if the new files doesn't exist (no overwrite, so not actually deleting anything anyway 1/2019
%(manual change)
...
if exist(FilePath)
    if exist(FilePath) %(manual change)-indent
...
if Paused == 1
    for SegmentCount = 1:NumberOfSegments
        [NumberOfChannels,LengthOfData{SegmentCount}] = size(NSx.Data{SegmentCount}); %(manual change)-indent
    end
elseif Paused == 0
...
elseif Paused == 1
    for SegmentNumber = 1:NumberOfSegments
        %Header
        %(manual_change) -SJ
        fprintf('%s\n','STOP! If you get here, go get SJ!');
        keyboard
        %This is the original line:        
        fwrite(FileID, NSx.RawData.DataHeader(1+9*(SegmentNumber-1))); 
        %This is what MEK replaced the line with:
        %fwrite(FileID, uint32(size(NSx.Data(SegmentNumber),2)));
        %    HACKY FIX ^^, not sure if this will fuck visualization up
