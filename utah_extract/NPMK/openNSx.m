...
%
%  7/2019.... CZ (from zaghloul lab) added a catch for infinite headers
%  "% 7/16/2019 CZ: pretty certain this means there is a problem with the file, 
%  though not sure how this loop works with the loop above that does the first pass of setting f.BOData and f.EOData
%
% 3/12/2020: SJ added 2 manual fixes from previous version 
%
% NOTE: to find manual changes, search for:
% %(manual change) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[FID, msg] = fopen([path fname], 'r', 'ieee-le'); %(manual change)
if FID < 1 %(manual change)
    error('%s; could not open file %s', msg, fname);
end
...
if NSx.RawData.PausedFile == 1
    while double(ftell(FID)) < f.EOF
        ...
        %(manual change)
        if headerCount > 10000 % 7/16/2019 CZ: pretty certain this means there is a problem with the file, though not sure how this loop works with the loop above that does the first pass of setting f.BOData and f.EOData
            fprintf('headerCount too high: for unknown reasons this file will stay in this loop forever. Quitting openNSx with error code\n');
            varargout{1} = -99;
            return;
        end        
    end
