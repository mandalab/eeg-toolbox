function [uniqueEegFilePaths] = changeEventsEegFile( subj, rootEEGdir, oldphrase, newphrase, silent )
%
...
behDir  = fullfile(rootEEGdir, subj, 'behavioral');


% Open text file that will log the results of this run
outText = fullfile(behDir, 'eventEegFilePathsTemp.txt');
fid     = fopen(outText, 'w+');
...
for bd=1:length(behDirs)
    ...
    expDir = fullfile(behDir, behDirs(bd).name);
    fprintf(     '\n--- searching %s\n', expDir);
    fprintf(fid, '\n--- searching %s\n', expDir);
    
    % Find what sessions were performed for this subject
    sessDirs = dir(fullfile(expDir, 'session_*'));
    if isempty(sessDirs)
    ...
    for sd=1:length(sessDirs)+1
        ...
        if sd<=length(sessDirs),
            evFile = fullfile(expDir,sessDirs(sd).name,'events.mat');   % session-by-session events.mat
            expStr = fullfile(behDirs(bd).name, sessDirs(sd).name);     % for reporting errors and/or total counts
        else
            evFile = fullfile(expDir,'events.mat');                     % experiment's root events.mat file (concatenated from each session)
            expStr = sprintf('root: %s', fullfile(behDirs(bd).name));   % for reporting errors and/or total counts
        end
...
fclose(fid);
outText2 = fullfile(behDir, 'eventEegFilePaths.txt');
