function [events] = RAM_FR_sys2_CreateTASKEvents(subject,expDir,session, sessName)
%function events = RAM_FR_sys2_CreateTASKEvents(subject,expDir,session,forceSESSION)
...
%thisSessDir = sprintf('session_%d',SESSION);  %- JW tweak
    ...
    switch upper(thisTYPE)
        ...
        case 'TRIAL'
            ...
            if strcmp(x{5}{1}, 'STIM') || ...
                    (strcmp(x{5}{1}, 'STIM_PARAMS') && x{6}>0)
                appendNewEvent_local(evCounter,'stimList',1);
                isStimList = true;
