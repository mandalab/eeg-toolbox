function [events] = RAM_FR_CreateTASKEvents(subject,expDir,session, sessName)
%function [events] = RAM_FR_CreateTASKEvents(subject,expDir,sessName,forceSESSION, startingElec)
%
% FUNCTION:
...
%
%
%  7/2017   jw tweaks to process for FRNU.  sessNum input --> sessName to allow for modified session name folders
%
...
while true
    ...
    switch upper(thisTYPE)
        ...
        case {'B','E','SESS_END','ORIENT', 'STIM_ON', 'REC_END', 'DISTRACT_START', 'DISTRACT_END','SESSION_SKIPPED',...
                ...
                'PRACTICE_REC_START','PRACTICE_REC_END','INSTRUCT_START','INSTRUCT_END','MIC','TRIAL_START'}
        ...
        case 'TRIAL'
            ...
            LIST = thisTRIAL;
            evCounter = evCounter + 1;
            if ~isempty(x{5}) && ~(strcmp(subject,'TJ083')||strcmp(subject,'TJ082')) % for backwards compat w/first two TJ pilot Ss
        ...
        case {'WORD_START'}
            ...
            %%- system3.0 version of the session.log
            x=textscan(thisLine,'%f %f %s %s %s %s %d %s %s %s %d','Delimiter',{'{','}','"',':',','}, 'MultipleDelimsAsOne',1);  % 1495549803042	0	WORD_START {"phase_type": "NON-STIM", "serialpos": 0, "word": "CONE", "listno": 1}

            thisWORD = removeDiacritics(x{9}{1});
            thisSP   = x{7};
            if ~isempty(x{5})
                thisStim = x{5}{1};
            else
            ...
            if isempty(itemNo),
                if strcmp(thisStim,'PRACTICE'); itemNo = -1;
                else error('word not found'); end
            end
            ...
            mkNewEvent_local(evCounter,x{1},x{2});
            appendNewEvent_local(evCounter,'type',x{5}{1});
            ...
            if currParamSet == 0
                ...
                appendNewEvent_local(evCounter,'stimList',1);
                 ...
                 %-------------------------------------------------------------------
        case {'PRACTICE_POST_INSTRUCT_START','PRACTICE_POST_INSTRUCT_END',...
                'WORD_END', 'TRIAL_END',...
                'ORIENT_START', 'ORIENT_END', 'ENCODING_START', 'ENCODING_END',...
                'RETRIEVAL_ORIENT_START', 'RETRIEVAL_ORIENT_END', 'RETRIEVAL_START', 'RETRIEVAL_END',...
                'WAITING_START', 'WAITING_END', 'ORIENT_START', 'ORIENT_END' }
            %- do nothing
        ...
        case {'RECOGNITION_INSTRUCT_START' 'RECOGNITION_INSTRUCT_END' 'RECOGNITION_START'}
            fprintf('\n WARNING: SKIPPING OVER RECOGNITION DATA');
            return;
        otherwise
            fprintf('\n WARNING: type ''%s'' not recognized',thisTYPE);
...
fid_foo1 = fopen(fullfile(expDir,'RAM_wordpool_noAcc.txt'),'r','native','latin1');
if fid_foo1==-1
    fid_foo1 = fopen(fullfile(expDir,'RAM_wordpool.txt'),'r','native','latin1');
    if fid_foo1==-1
        error('RAM_wordpool.txt not found')
    end
end
X = {};
while true
