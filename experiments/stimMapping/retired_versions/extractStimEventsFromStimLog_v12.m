...
if length(offset)==0,
    fprintf('\n EMPTY STIM LOG... probably need to delete session folder: %s',sessDir);
    keyboard
    events = struct([]);
    return;
end
...
while idx <= size(offset,1)
    ...
    events(evcnt).offset = str2num(offset{idx}) + 3600000*addHour; %- JW converted this to num 8/2017
    events(evcnt).mstime = str2num(offset{idx}) + 3600000*addHour; %- JW converted this to num 8/2017 Was curTimePos{idx} previously - DY
    events(evcnt).mstime2 = str2num(offset2{idx}) + 3600000*addHour;
    events(evcnt).mstime3 = str2num(offset3{idx}) + 3600000*addHour;
    ...
    elseif contains(type{idx},'MSeq')
        ...
        events(evcnt).MSeqElecList = unique(sitesbyName(~cellfun('isempty',sitesbyName)));
