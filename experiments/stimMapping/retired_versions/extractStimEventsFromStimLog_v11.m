...
% header = textscan(fid,'%s%s%s%s%s%s%.1f%s%s',1,'headerLines',6); %- get the the file header
% fclose(fid);
GUI_ver = 7; % Hard coding to 7 for conditional statement below

% Old way: [offset sites amp1 amp2 freq pulses type annotation]=textread(stimFile,'%s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t');  %- get the file contents
...
%curTimePos = str2num(offset{7});
curTimePos = str2num(offset{1}); %- use numHeaderLines above to skip the junk.. so first value should be fine
...
addHour = 0; % Just to get through the code
...
if length(fNamePulse)>0
    ...
    if ~exist(pulseFile,'file'),
        fprintf('\n uh oh... missing pulse file %s',pulseFile),
        keyboard;
    end
    while 1
        tline = fgetl(fid_orig);
...
% Get rid of -999-999 if it's there
sitepairs(ismember(sitepairs,'-999-999')) = '';
...
sitesbyNameList = unique(sitesbyName(~cellfun('isempty',sitesbyName)));  %- create this list once and copy over and over below
sitesByNameBins = zeros(1,length(sitesbyNameList));
...
events = struct([]);  numPass=0;
while idx < size(offset,1)
    
    if contains(offset{idx},'-999')
        idx = idx + 1;
        continue
    end
    
    numPass=numPass+1;
    if mod(numPass,floor(size(offset,1)/20))==0 fprintf('.'); end %- show progress... this can be slow
    ...
    events(evcnt).cerestimID = str2double(id{idx});
    ...
    events(evcnt).mstime = str2num(offset{idx});  %- JW converted this to num 8/2017 Was curTimePos{idx} previously - DY
    events(evcnt).mstime2 = nan; %- add this field so the events can be merged with v12
    events(evcnt).mstime3 = nan;
    ...
    elseif contains(type{idx},'MSeq')
        ...
        events(evcnt).MSeqElecList = sitesbyNameList; %- JW changed this so not calling unique on a large cell array each loop iteration
         
        % Figure out how many electrodes were stimulated at once
        numAtThisTime = ismember(offset,offset{idx});
        ...
        events(evcnt).MSeqElecBin = sitesByNameBins;
        for i = 1:length(elecsOn);
            events(evcnt).MSeqElecBin(strcmp(sitesbyNameList,elecsOn{i}))=1; %- JW change this to try to make the loop quicker
        end
        %for i = 1:length(events(evcnt).MSeqElecList)
        %    events(evcnt).MSeqElecBin(i) = max(contains(elecsOn,events(evcnt).MSeqElecList(i)));
        %end
...
hasDC09=0; hasDC10=0; hasELOG=0; %- force an update of these each time, just like the session log
...
if hasELOG==0,
    ...
    if exist([sessDir '/eeg.eeglog.up']), delete([sessDir '/eeg.eeglog.up']); end
end
