function [events] = extractStimEventsFromStimCSV_v2(subject, sessName, behDir, eegDir)
...
for iSub = 1:numSubSess
    ...
    if stimCSV.id(find(stimCSV.subSession == iSub)) == microID
        ...
        sitepair2 = unique(stimCSV.site2(find(stimCSV.subSession == iSub)));
        % Get rid of NaN if it's there
    ...
    for n = 1:length(sitepair1)
        ...
        if stimCSV.id(find(stimCSV.subSession == iSub)) == microID
            sitepairs{iSub}(n) = horzcat(stimNames{:});
        else
...
while idx <= size(stimCSV.offset,1)
    ...
    elseif contains(stimCSV.type(idx),'MSeq') || contains(stimCSV.type(idx),'ClinicalGroupStim') || contains(stimCSV.type(idx),'PulsesMaxOnly')
...
if hasDC09==0,
    fNamePulse = dir([sessDir '/eeg.eeglog']);
    if length(fNamePulse)>0,
        
        %- create an eeg.log with python/unix time for prepAndAlign
        eegLogFile = [sessDir '/eegDC09.eeglog'];
        eegLogFileUp = [sessDir '/eeg.eeglog.up'];
        fileId =fopen(eegLogFile,'w');      % for now just replacing the eeg.eeglog.up file
        fileId2 =fopen(eegLogFileUp,'w');      % for now just replacing the eeg.eeglog.up file
        ...
        [timePCstr, ~, pulseStr]=textread(pulseFile,'%s\t %s\t %s');  %- get the file contents
        
        %- now loop through all the "up" times and copy them to eeg.log with corrected mstime
        iUp = find(strcmp(pulseStr,'CHANNEL_0_UP'))';
        for ii=iUp,
            curTimePos = num2str(str2num(timePCstr{ii}) + 3600000*addHour);
            
            fprintf(fileId,'%s\t%d\t%s\n',curTimePos,0,'CHANNEL_0_UP');
            fprintf(fileId2,'%s\t%d\t%s\n',curTimePos,0,'CHANNEL_0_UP');
