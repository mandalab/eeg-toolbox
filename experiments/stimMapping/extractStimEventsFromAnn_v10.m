function [events] = createStimEventsFromAnn(sessLogFile, subject, sessionName, sessionNum,eegDir)
%function [events] = createStimEvents_v3a(sessLogFile, subject, sessionName, sessionNum,eegDir)  %% Old name
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%  Function for extracting behavioral data from stimMapping session. %%%%
%%%%
%
% 8/2017 -- JW renamed to "createStimEventsFromAnn_v10.m"  to make clear how this differs from newer GUI-based stimMapping extraction
%
% update 1/10/16
...
if (fid==-1)
    ...
    fprintf('\n   Session.log not found: %s, so creating now', sessLogFile);
    ...
    if modFile==0  &  subjNum<40,
        fprintf('\n  *** Zero automatic modifications were made: must do it by hand, else this isnt a valid stimMapAnn session (likely for >NIH040) ***');
...
fclose(fid_error);
if fixCount,
    fprintf('\n   Fix %d annotations in %s session.log (DC10 updown) and rerun\n', fixCount, sessionName'); 
else
