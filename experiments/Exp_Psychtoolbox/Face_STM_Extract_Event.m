...
function [events] = Face_STM_Extract_Event(behDir,sessionName)
% analyze the data
% Zane 2014.2.11
    ...
    for ise = 1:length(SessionID)
        ...
        for iblock = 1:nblock
            for itr = 1:ntrail
                ...
                ToRem(itr+(iblock-1)*ntrail,isub) = datawhole{itr+(iblock-1)*ntrail,isub}.MemAll(1); % always the first one is to be remember
                ToForget(itr+(iblock-1)*ntrail,isub) = datawhole{itr+(iblock-1)*ntrail,isub}.MemAll(2); %  always the second one is to be forgotten 
                TestItem(itr+(iblock-1)*ntrail,isub) = datawhole{itr+(iblock-1)*ntrail,isub}.TestItem; %  1 First Gen, 2 recent Gen
