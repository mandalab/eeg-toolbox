function events = extractDoorsEvents(sessLogFile, subject, sessionName, priorEvents)
                        ...
                        case 0
                            switch onScreen
                                case 'IMAGES_ON_SCREEN'
                                    xTOT=textscan(thisLine,'%f%d%s%s%s%d%s%s%s%s%d%s%d%s%.6f');
                                    presentationNum = xTOT{11}(1); 
                                    samediff        = xTOT{13}(1);
                                    rewProb        = xTOT{15}(1);
                                case 'IMAGES_ON_SCREEN_RESP'
                                    xTOT=textscan(thisLine,'%f%d%s%s%s%d%s%s%s%s%s%s%s%s%s%s%d%s%d%s%d%s%.2f%d');
                                    corrResp = xTOT{11}{1};
                                    subRespParseStr  = xTOT{13}{1};
                                    subResp = subRespParseStr(isstrprop(subRespParseStr,'alpha'));
                                    subRespKey = xTOT{15}{1};
                                    if strcmp(subRespKey,'LEFT'),respLoc=1;elseif strcmp(subRespKey,'RIGHT'),respLoc=2;else respLoc=3;end
                                    correct  = xTOT{17}(1);
                                    RT       = xTOT{19}(1);
                                    samediff        = xTOT{21}(1);
                                    rewProb        = xTOT{23}(1);
                        ...
                        case 1                    
                            switch onScreen
                                ...
                                case 'IMAGES_ON_SCREEN_RESP'
                                    ...
                                    xTOT=textscan(thisLine,'%f%d%s%s%s%d%s%s%s%s%s%s%s%s%s%s%s%s%d%s%d%s%d%s%.2f');%[greenbrown]%*[]]%*["]%[greenbrown]%*3c%*["]%[boygirl]%*2c%s%s%[^]"]%s%s%s%s%d%s%d%s%d%s%.2f%d');%,'delimiter','[]','MultipleDelimitersAsOne',true);
                                    
                                    if strcmp(xTOT{9}{1},'CORR_RESP')
                                        xTOT=textscan(thisLine,'%f%d%s%s%s%d%s%s%s%s%s%s%s%s%s%s%s%d%s%d%s%d%s%.2f');
                                        ...
                                        subRespKey = xTOT{16}{1};
                                        correct  = xTOT{18}(1);
                                        RT       = xTOT{20}(1);
                                        samediff        = xTOT{22}(1);
                                        rewProb        = xTOT{24}(1);                                        
                                        
                                    else    %have fixed logging, only this else block will be needed 
                                        corrRespParseStr1 = xTOT{11}{1};
                                        corrRespParseStr2 = xTOT{12}{1};
                                        corrResp = [corrRespParseStr1(isstrprop(corrRespParseStr1,'alpha')) corrRespParseStr2(isstrprop(corrRespParseStr2,'alpha'))];
                                        subRespParseStr1  = xTOT{14}{1};
                                        subRespParseStr2  = xTOT{15}{1};
                                        subResp = [subRespParseStr1(isstrprop(subRespParseStr1,'alpha')) subRespParseStr2(isstrprop(subRespParseStr2,'alpha'))];
                                        subRespKey = xTOT{17}{1};
                                        correct  = xTOT{19}(1);
                                        RT       = xTOT{21}(1);
                                        samediff        = xTOT{23}(1);
                                        rewProb        = xTOT{25}(1);
                                    ...
                                    msDuration = RT; %check this
%                                     disp(corrResp)
%                                     disp(subResp)
%                                     disp(correct)
%                                     disp(RT)  
                                case 'TARGET_FOUND'
                                    xTOT=textscan(thisLine,'%f%d%s%s%s%d%s%s%s%d%s%d%s%.2f');
                                ...
                                case 'REWARD'
                                    xTOT=textscan(thisLine,'%f%d%s%s%s%d%s%.6f%s%d%s%.2f%s%d');
                                    rewVal = xTOT{8}(1);
                                    samediff = xTOT{10}(1);
                                    rewProb = xTOT{12}(1);
                                    rewReceived = xTOT{14}(1);   
                            end
                    end
