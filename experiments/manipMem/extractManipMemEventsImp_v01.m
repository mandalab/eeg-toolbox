function [events] = extractManipMemEventsImp_v01(subject, behDir, sessionName)
...
while 1
    ...
    switch eventType
        case 'ENCFIX'
            ...
            thisTrial = thisLineStr{6};
            
        case 'ENCIMG'
            makeNewEvent = 1;
            thisTrial = thisLineStr{6};
            [~,name,ext] = fileparts(thisLineStr{8}); thisEncImg = [name ext];
            [~,name,ext] = fileparts(thisLineStr{10}); thisRetImg = [name ext];
    ...
    if makeNewEvent
        clear thisEvent
	thisEvent.experiment = 'manipMemImp';
