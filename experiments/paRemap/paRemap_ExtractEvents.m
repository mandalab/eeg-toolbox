...
annFromProbeOn = 0;
annFileExists = 0;  %- assume no annotation... this was the case for NIH029
type = 0;
while true
...
%             end
            ...
            while true && annFileExists
                ...
                %- Generic text scan to get time, offset, and type
                ann_words = strsplit(line);
                
                % add to list these annotation data
                if length(ann_words) == 2
                    thisVocWord = ann_words{2};
                else
                    thisVocWord = ann_words{3};
                end
                
                if annindex == 0
                    timeAfterRec          = str2double(ann_words{1}) + recStart;   %- must add (1) because numbers after the string in the above line cause overflow to first %f
                    vocalizedWord         = {thisVocWord};
                    annindex = annindex + 1;
                else
                    timeAfterRec          = [timeAfterRec; str2double(ann_words{1}) + recStart];   %- must add (1) because numbers after the string in the above line cause overflow to first %f
                    vocalizedWord         = [vocalizedWord; thisVocWord];
                    annindex = annindex + 1;
                end
            end
        ...
        case {'BLOCK_0', 'BLOCK_1', 'BLOCK_2', 'BLOCK_3', 'BLOCK_4', 'BLOCK_5'}
            ...
            %- NIH029 still saved the practice trials... we stopped doing that >NIH030, so just jump past them here    
                %fprintf('\n Heads up... older version of code saved practice in session.log, jump to TEST and then step back one line');
                ...
                while inPractice,
                    ...
                    blockTOT = textscan(thisLine, '%f%d%s%s%s');
                    if length(blockTOT{5})>0 && strcmp(blockTOT{5}(1), 'TEST'),
                        fseek(fid,-length(thisLine)-1,'cof'); %- put back the "TEST" line so it can be read by fgetl at top of this loop
                        inPractice = 0;
                    end
                end
            end
        ...
        case {'MATCHWORD_ON'}
            ...
            if annFileExists % make sure there is an annotation file
                ...
                if ~exist('timeAfterRec', 'var')
                    error('No annotations made: %s\n', annfilename);
                end
