function [events] = extractReplayTaskEvents(rootEEGdir, subject, sessionName)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% Function for extracting behavioral data for replayTask %%%%%%%%%%%%% 
   
% Created by Jiali Zhang 12/2020
% Updated by Jiali Zhang 02/2021 (MOST UPDATED VERSION)
% Inputs:
%   rootEEGdir: eeg directory path 
%   subject: e.g. 'NIHXXX'
%   sessionName: e.g. 'session_0'

% Version 2: NIH087 and onwards
...
% sessionDir = '/Users/zhangj34/Desktop/replayTask/NIH088/session_2';
...
%     stimDurms     = nan; 
    ...
    TRIAL = sprintf('TRIAL_%d,', (0:24)); % create var TRIAL to store all trial names 'TRIAL_0' etc...
    TRIAL = split(TRIAL, ','); 
    DEC_Q = sprintf('DECISION_QUESTION_%d,', (0:12));
    DEC_Q = split(DEC_Q, ','); 
...
%                     index = index + 1; 
                ...
                case {'TARGET_REST_START', 'TARGET_REST_END'}
                    
                    blockType = 'REST_DEC'; 
                    if contains(block, 'TARGET_REST_START')
                        blockStartms = mstime; 
                        index = index + 1;    
                    elseif contains(block, 'TARGET_REST_END')
                        events(end).blockEnd = mstime;
                        events(end).blockDuration = events(end).blockEnd - events(end).blockStart; 
                    end 
