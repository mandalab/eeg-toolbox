function [events] = stimGuess_ExtractEvents_v3(sessLogFile, subject, sessionName)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%
%  VERSION 3:  added support for UP/DOWN/LEFT/RIGHT arrows (NIH072 physio).  Code should work fine with older subjects, but saving as new version just in case
%
% JHW 3/2017
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%subject     = 'NIH072'; sessionName = 'session_1';
%subject     = 'NIH072'; sessionName = 'session_0';
%subject     = 'NIH072_preOp'; sessionName = 'session_5';
%subject     = 'NIH072_preOp'; sessionName = 'session_6';
%sessLogFile = sprintf('/Users/wittigj/Desktop/experiments/stimulusGuess/trunk/data/%s/%s/session.log',subject,sessionName);
...
stimCats    = {'NOISE','ANIMAL','OBJECT','PERSON','PLACE','BUSH','CLINTON','KENNEDY','TRUMP','UP','DOWN','LEFT','RIGHT'};
respTypes   = {'CORRECT(hit)','INCORRECT(fa)','INCORRECT(skip)'};

%- key for conversion from string to number
catStr2Num  = {'ANIMAL', 'OBJECT', 'PERSON', 'PLACE','BUSH','CLINTON','KENNEDY','TRUMP','','NOISE','UP','DOWN','LEFT','RIGHT'}; %- make 'NOISE' a 10
catStr2NumB = {'ANIMAL', 'OBJECT', 'PERSON', 'PLACE','BUSH','CLINT',  'JFK',    'TRUMP','','',     'UP','DOWN','LEFT','RIGHT'}; %- make 'NOISE' a 10
...
respCatList   = {'DOWN(ANIMAL)', 'RIGHT(OBJECT)', 'UP(PERSON)', 'LEFT(PLACE)','DOWN(BUSH)','UP(CLINTON)','LEFT(KENNEDY)','RIGHT(TRUMP)','UP(UP)','DOWN(DOWN)','LEFT(LEFT)','RIGHT(RIGHT)'}; %- NIH072 and beyond... add "up/down/left/right"
%respCatList   = {'DOWN(ANIMAL)', 'RIGHT(OBJECT)', 'UP(PERSON)', 'LEFT(PLACE)','DOWN(BUSH)','UP(CLINTON)','LEFT(KENNEDY)','RIGHT(TRUMP)'}; %- NIH064-72
...
blockNumXLS   = nan;      %- assume xls block number not saved to session log
...
while 1
    ...
    switch eventType
        ...
        case {'BLUR_DIR'}
            ...
            if contains(thisSessBlurDir,'ninetyBlur'),
                ...
                if contains(thisSessBlurDir,'->'), %- V5 session hack
                    thisSessBlurDir = 'multiBlur'; thisSessBlurLevel='AbsoluteVal_5ish'; %- call it 5... but really it is a mix of 0 and 5
                else
        ...
        case {'BLOCK_START'}
            if length(thisLineStr)<5 || ~contains(thisLineStr{5},'READ_XLS_BLOCK'),
                fprintf('\n get john... shouldnt happen');
                keyboard;
                blockNumXLS = nan;
            else
                blockNumXLS = str2num(thisLineStr{5}(16:end)); %- skip past "READ_XLS_BLOCK_"
            end
        ...
        case {'IMG_ON_SCREEN'}
            ...
            if isNinteyBlurSess | strcmp(thisCategory, 'NOISE') | thisCategoryNum>10,
    ...
    for ii=1:makeNewEvent,
        ...
        thisEvent.blockNumXLS       = blockNumXLS;   %- block being read from XLS. if >1000 then this is arrow baseline.
...
%xLine = 0.5+[0:60:240 240+[15:15:120]]; %- not all arrows are shown each block, so the xVal set by iStim does not necessarily go by steps of 15
...
subplot(212)
hist([events.durationImg],200)
set(gca,'tickdir','out','fontsize',fS,'box','off')
xlabel('Stim Duration (ms)')

subplot(211)
hist([events.rxnTime],200); hold on;
plot(median([events.durationImg])*[1 1],get(gca,'ylim'),'r--','linewidth',2);
set(gca,'tickdir','out','fontsize',fS,'box','off')
...
%subplot(211)
imagesc(1:length(stimIDs),blocks,respByStim,[0 14])
set(gca,'xtick',[30 90 150 210 240+[7:15:60]],'xticklabel',catStr2NumB(1:8),'tickdir','out','box','off','fontsize',fS); hold on;
colormap([1 1 1; lines(8); 1 1 1; 1 1 1; lines(4)]);
colorbar
title(strTitle);
ylabel('Actual Response vs Stimulus ID');
...
ylabel('Rxn Time vs Stimulus ID');
for iL=1:length(xLine), plot([1 1]*xLine(iL),get(gca,'ylim'),'k--','linewidth',2); end
