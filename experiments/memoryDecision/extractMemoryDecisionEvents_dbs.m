function events = extractMemoryDecisionEvents(subject, expDir, session, priorEvents)
...
% subject     = 'DBS051_preop';%'DBS049_left';   % 
% sessionName = 'session_2';
% 
% sessionDir  = fullfileEEG(rootEEGdir,subject,'behavioral/memoryDecision',sessionName);
...
sessionName = ['session_' session]; 
sessionDir  = fullfileEEG(expDir,sessionName);
sessLogFile = fullfileEEG(sessionDir,'session.log');
priorEvents = [];
...
while true
    ...
    switch type
        ...
        case 'BLOCK_START'
            ...
            % which aren't
            try
            categFileName = sprintf('block%d_CategoryInfo_vDBS_1.0.csv',block-1);
            blockCategFullFile = fullfileEEG(sessionDir,categFileName);
            Categs = textread(blockCategFullFile,'%s','delimiter',',');
            catch
            categFileName = sprintf('block%d_CategoryInfo_v1.0.csv',block-1);
            blockCategFullFile = fullfileEEG(sessionDir,categFileName);
            Categs = textread(blockCategFullFile,'%s','delimiter',',');                   
            end
...
%             block_test_FileName = sprintf('block%d_test_v3.3.csv',block);
        ...
        case {'B','ITI_START',...
                ...
                'SESS_DURATION_ACTIVE', 'SESS_DURATION_TOTAL','BLOCK_END','BLOCKS_DURATION_ACTIVE', 'You','E'}
