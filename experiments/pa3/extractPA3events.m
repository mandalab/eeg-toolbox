...
%
%  (5) JHW 7/2017 tweaked mstime output for passes... was nan but that messed up alignemnt.  switched to actual mstime
%
            ...
            switch upper(testType)
                case 'START'
                    ...
                    if fseek(fid2,1,'bof')==-1 %annotation file is empty
                        ...
                        evCounter = evCounter + 1;
                        if ~isnan(thisRT), msTimeIn = thisMSTIME+thisRT;  % JHW added 7/2017 when reprocesssing old pa3 sessions
                        else               msTimeIn = thisMSTIME; end
                        mkNewEvent_local(evCounter,msTimeIn,20)
