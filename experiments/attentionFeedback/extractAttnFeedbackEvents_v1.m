...
% clear all
% 
% rootEEGdir  = '/Volumes/EEG_56STAGE/stage/grabbed';
% %rootEEGdir  = 'C:/Users/jDub/DataJW/eeg/';
% subject     = 'NIH081';   % EEG002  NIH016
...
testCount       = nan ;                 % ? 
newImage        = nan ;                 % 1 if this is a new image on the screen, updated if previous image's feedback score was above threshold to "uncover" part of image
...
while true
    ...
    newImage            = newImage  ;   %- 1 if this is a new image on the screen, updated if previous image's feedback score was above threshold to "uncover" part of image
    ...
    switch type
        ...
        case 'FEEDBACK_SCORE'
            xTOT=textscan(thisLine,'%f%d%s%.1f');
            feedbackScore = xTOT{4};
...
%             events(indexFRstart).msDuration      =  mstime - events(indexFRstart).mstime; %apply this to FREE_RECALL_START
        ...
        case {'SUMMARY_BLOCK'}
            ...
            newImage        = nan     ;     %reset the newImage flag at end of block
        case {'BLANK'}
    ...
    if index>length(events),
        ...
        thisEvent.blockType         = blockType   ;   % testing or training
        ...
        thisEvent.newImage        = double(newImage)      ;   %- 0 for no, 1 for yes
