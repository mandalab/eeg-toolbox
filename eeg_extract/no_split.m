...
% Change these variables here and lower down (line 19, c_reref=[1 2;...)) if there were skipped channels
spike_index=1:3;
LFP_index=4:6;
...
if setdiff(c_reref,[spike_index LFP_index ecogchannels_index TTL_index]); error('Zar: you can not reref channels that dont exist. Change c_reref'); end % make sure you've edited lines 14,15,16, and 19 correctly. 
...
for index=1:length(dataFiles)
    ...
    if index>1 && (CMacro_LFP_01_TimeBegin-prevfilend)>1 %if longer then 1 second between files, split into two. 1 second cut off arbitrarily chosen
        ...
        samprate_ecog=rawData.CECOG_HF_1___01___Array_1___01_KHz*1000;
        samprate_ecog_new=1000;
        [fsorig_ecog, fsres_ecog] = rat(samprate_ecog/samprate_ecog_new);
        for ecogchan=ecogchannels_index
            eval(['chan{ecogchan}=[chan{ecogchan} resample(double(rawData.CECOG_HF_1___' num2str(ecogchan-ecogchannels_index(1)+1, '%.2d') '___Array_1___' num2str(ecogchan-ecogchannels_index(1)+1, '%.2d') '),fsres_ecog,fsorig_ecog)];']);
        end
