...
outputDir=fullfile(homeDir,subject,'docs');
    ...
    if exist(fullfile(homeDir,subject,'docs','README.txt'))
        ...
        coor_begin=find(strcmpi(read_me,'UPDRS'));
        read_me=read_me(coor_begin:end); % get rid of the opening paragraph of the read me. 
    else
    ...
    % target (STN or GPi)
    coor1=find(strcmpi(read_me,'stn')); coor2=find(strcmpi(read_me,'gpi')); coor3=find(strcmpi(read_me,'vim/vc'));
    if ~isempty(coor1)
        coor=coor1;
    ...
    elseif ~isempty(coor3)
        coor=coor3;
        DBS_params.targets(1).name=cell2mat([read_me(coor(1)-1) ' ViM/Vc']);
        DBS_params.targets(2).name=cell2mat([read_me(coor(2)-1) ' ViM/Vc']);
    end
