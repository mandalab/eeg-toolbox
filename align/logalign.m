...
%
%  12/2018 -- jw tweaked the criterion for throwing an error that requires hitting the keyboard
%
%
% FUNCTION:
...
alignFigPath = fullfileEEG(path,'pulseAlignFig.png'); 
...
for f = 1:length(eeg_offset)
    ...
    [path,chan] = fileparts(eeg_files{f});
    [~,folder]  = fileparts(path);
    fileonly{f} = chan;   %- old data structure, file name contained date string
    fileonly{f} = folder; %- new data structure: channels all contained within folder with date string... use that string in the session.log.align file
    fileroot{f} = path;
    ...
    %     RULE:  if 1% of the deviations >10ms, then throw a warning and force a stop.  
    %            Otherwise dont stop but make an alignment fig for each
    %if maxdev>maxDevThresh, strWarning = sprintf('%s WARNING: max deviation (%f ms) is >5 ms\n', strWarning, maxdev);    end;
    %if pcntAboveDevThr>5,   strWarning = sprintf('%s WARNING: deviations exceed %.1fms more than 5%% of the time (%.1f%%; probably should reject session)\n', strWarning, maxDevThresh,pcntAboveDevThr);    end;
    %if pcntAbove2DevThr>1,  strWarning = sprintf('%s WARNING: deviations exceed %.1fms more than 1%% of the time (%.1f%%; probably should reject session)\n', strWarning, maxDevThresh*2,pcntAbove2DevThr);    end;
    if pcntAbove2DevThr>1 & pcntAboveDevThr>5,  
        strWarning = sprintf('%s WARNING: deviations exceed %.1fms more than 5%% of the time AND more than %.1fms 1%% of the time (probably should reject session)\n', strWarning, maxDevThresh,maxDevThresh*2);
    end;
    if stats(1)<0.98,  strWarning = sprintf('%s WARNING: regression R^2 (%f)   is <0.98\n', strWarning, stats(1));  end;
    if length(strWarning)>0  |  maxdev>maxDevThresh,
        hFig = figure(100); clf
        set(gcf,'name','Alignment Regression Residuals','color','w')
        hist(rawDevs{f},100);
        set(gca,'fontsize',15,'tickdir','out','box','off');
        xlabel('regression residuals (ms)');
        ylabel('occurances');
        strTitle = sprintf('Alignment: %s',strTaskSess);
        strTitle(find(strTitle=='_'))='-';  %replace underscore with dash to avoid tex interpreter error
        title(strTitle,'fontsize',15)
        hold on
        plot([1 1]*-maxDevThresh,  get(gca,'ylim'),'r--');
        plot([1 1]* maxDevThresh,  get(gca,'ylim'),'r--');
        plot([1 1]*-maxDevThresh*2,get(gca,'ylim'),'r--');
        plot([1 1]* maxDevThresh*2,get(gca,'ylim'),'r--');
        try
           fig2pngSimple(hFig,alignFigPath); %- new version... just save the fig (no screen shot)
        catch
            fprintf('\n attempted to save alignment figure to session folder failed.');
        end
    end
    ...
    strStats = sprintf('%s\tDeviations>%.1fms  = %.1f%%\n', strStats, maxDevThresh,pcntAboveDevThr);
    strStats = sprintf('%s\tDeviations>%.1fms = %.1f%%\n', strStats, maxDevThresh*2,pcntAbove2DevThr);
...
for f = 1:length(log_files)
    ...
    if strfound(log_files{f},'.mat')
        ...
        %         % save a backup of the file -- JW diabling this function... no reason to do this
        %         try
        ...
        % load the file
        [ms,therest] = textread2(log_files{f},'%n%[^\n]','delimiter','\t');
    ...
    if  strfind(log_files{f},'eeg.eeglog')
        % make it a very high threshold for throwing a warning with eeg.eeglog events not aligned
        if  numEventsTotal > numEventsAligned*2,
            strWarning = sprintf('%s WARNING: more than half (%d of %d events) in %s occurred beyond duration of EEG file \t[check %s]\n', strWarning, numEventsSkipped, numEventsTotal,log_file_name, check_file_name);
        end
    elseif numEventsTotal > numEventsAligned
        %- for session.log and events.mat, dont let ANY events fall out of alignment
    ...
    for l = 1:length(ms)
        ...
        if isempty(ef)
            ...
            if dostruct
                ...
                events(l).eegfile = fileroot{ef};                                        % new version 7/2017 --- use fileRoot because now all split files are contined within a folder, e.g.,  '/Volumes/JW24TB/data24TB/eeg_new/NIH032/eeg.noreref/150728_1913'
