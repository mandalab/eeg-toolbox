function eegRawServerDir( subj, rootEEGdir, rawFilePrefix, strDateStart, strDateEnd, nktdir )
...
%      -nktdir [optional]- directory of Dr. Inati's EEG server, or string for Archive subdirecotry. If unpassed, a default is defined
%                   e.g., '/Volumes/Shares/EEG/LTVEEG_DATA/Archive/NKT/EEG2100'
%                   e.g., 'Archive4'   -->  '/Volumes/Shares/EEG/LTVEEG_DATA/Archive4/NKT/EEG2100'
%
%   OUTPUTS:
...
%- user passed in a directory... check to see if just archive sub-directory or actual full directory
if exist('nktdir', 'var'),
    if ~exist(nktdir,'dir'),
        nktdir = fullfileEEG('/Volumes/Shares/EEG/LTVEEG_DATA/',nktdir,'NKT/EEG2100');
    end
end

%- user passes nktdir, but if not, try the one(s) in here
if (~exist('nktdir', 'var') || ~exist(nktdir, 'dir')) 
    nktdir = '/Volumes/Shares/EEG/LTVEEG_DATA/nkt/EEG2100';
    %nktdir = '/Volumes/Shares/EEG/LTVEEG_DATA/nkt/EEG2100-full';
    %nktdir = '/Volumes/Shares/EEG/LTVEEG_DATA/Archive/NKT/EEG2100'; %- use this for old archived files
    %nktdir = '/Volumes/Shares/EEG/LTVEEG_DATA/Archive2/NKT/EEG2100'; %- use this for old archived files
    %nktdir = '/Volumes/Shares/EEG/LTVEEG_DATA/Archive4/NKT/EEG2100'; %- use this for old archived files
    %nktdir = '/Volumes/Shares/EEG/LTVEEG_DATA/Archive5/NKT/EEG2100'; %- use this for old archived files
    fprintf('Using default directory: %s\n', nktdir);
end
...
if getEmbeddedDates==1,
    ...
    rawDateN = [rawListCopy.dateActNum];
