function [dset] = surf_load_dset(varargin)
...
for my_hemi = {'lh','rh'}
    dset.hemi = my_hemi{:};
    both_dset.(my_hemi{:}) = dset_load(dset);
end
return
...
% build the path
dset.name = dset_build_filename(dset.surfRez, dset.hemi, dset.dset);
...
return
function dset_name = dset_build_filename(surfRez, hemi, dset)
switch surfRez
    case 'native'
        surfRez = '';
    otherwise
...
dset_name = strcat(surfRez, hemi, '.', dset,'.niml.dset');
return